...
class TestLoadDriverData:
    ...
    def test_handles_json_decode_or_other_exception(self, mock_file, mock_exists):
        ...
        def raise_on_load(*args, **kwargs):
            raise ValueError("bad json")  # caught by except Exception

        # Make json.load raise; easiest is patching json.load, but here raise when file read attempted:
...
class TestLoadData:
    ...
    def test_full_pipeline(self, mock_load_driver, mock_merge_team, 
                          mock_merge_entries, mock_load_entries, mock_load_standings):
