...
def train_models(df):
    ...
    print(
        f"Training subset: {len(x_train_sub)} samples, "
        f"{y_train_sub.sum()} promotions ({y_train_sub.mean():.2%})",
    )
    print(
        f"Validation: {len(x_val)} samples, {y_val.sum()} "
        f"promotions ({y_val.mean():.2%})",
    )
    print(
