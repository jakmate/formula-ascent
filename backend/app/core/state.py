...
class AppState:
    ...
    def save_state(self):
        ...
        state = {
            "last_scrape_full": (
                ...
                if self.system_status["last_scrape_full"]
                else None
            ),
            "last_scrape_predictions": (
                self.system_status["last_scrape_predictions"].isoformat()
                if self.system_status["last_scrape_predictions"]
                else None
            ),
            "last_scrape_schedule": (
                self.system_status["last_scrape_schedule"].isoformat()
                if self.system_status["last_scrape_schedule"]
                else None
            ),
            "last_training": (
                self.system_status["last_training"].isoformat()
                if self.system_status["last_training"]
                else None
            ),
    ...
    def load_state(self):
        ...
        try:
            ...
            log.exception("Corrupted state file. Reinitializing state.")
            # Backup corrupted file
            STATE_FILE.rename(f"{STATE_FILE}.backup")
            return False

        except (KeyError, OSError, AttributeError, ValueError):
            log.exception("Error loading state")
