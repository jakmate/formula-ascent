...
def analyze_features(df, feature_cols, temporal_split_col="year", seed=SEED):
    ...
    try:
        ...
        vif_data = pd.DataFrame({"feature": X.columns, "VIF": [np.nan] * len(X.columns)})
    ...
    years = df[temporal_split_col].unique()
    years = sorted(years)
    n_val_years = max(1, int(len(years) * 0.2))
    val_years = years[-n_val_years:]

    train_mask = ~df[temporal_split_col].isin(val_years)
    val_mask = df[temporal_split_col].isin(val_years)
    ...
    for group in redundant_groups:
        ...
        print(
            f"   → Keep: {best_in_group} (perm_importance={group_df.loc[group_df['feature']==best_in_group, 'perm_importance'].iloc[0]:.4f})"  # noqa: 501
        )
        for f in group - {best_in_group}:
            print(f"   → Remove: {f}")

    # Also remove features with non-positive permutation importance
    non_predictive = importance_df[importance_df["perm_importance"] <= 0]["feature"].tolist()
    non_predictive = [f for f in non_predictive if f not in to_remove]  # avoid duplicates
    if non_predictive:
        print("\n🗑️  Non-predictive features (perm_importance ≤ 0):")
        for f in non_predictive:
            imp = importance_df.loc[importance_df["feature"] == f, "perm_importance"].iloc[0]
            print(f"   • {f} ({imp:.4f})")
